...
if(isset($_GET['id'])) {
    ...
    $sql = "SELECT id, montant, typpe, IdCompte FROM transactions WHERE id = ?";
    ...
    if (isset($_POST['updateTransactions'])) {
        $updateTransactions = new Transactions($conn);
        $updateTransactions->setId($transactionsIdToUpdate);     

         $updateTransactions->setMontant(isset($_POST['montant']) ? $_POST['montant'] : '');
        $updateTransactions->setType(isset($_POST['typpe']) ? $_POST['typpe'] : '');
        $updateTransactions->setAccountId(isset($_POST['IdCompte']) ? $_POST['IdCompte'] : '');
    
        // Call the updateTransactions function
...
<body>
    ...
    <div class="flex flex-col justify-center items-center h-screen">
        <form action="transactionEdit.php?id=<?= $transactionsIdToUpdate; ?>" method="POST" class="w-[500px] h-[340px] mx-auto bg-cyan-800">
        <h1 class="text-3xl text-white font-bold flex flex-col justify-center mt-5 items-center">Add Account</h1>
            ...
            <label for="typpe" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0]  rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-orange-600 peer-focus:dark:orange-orange-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">TYPE</label>
                <select value="<?= $transactionsData['typpe']; ?>"  class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-400 dark:focus:border-orange-600 focus:outline-none focus:ring-0 focus:border-orange-600 peer" name="typpe" id="typpe">
                    <option class="text-gray-800" value="Credit">CREDIT</option>
                    <option class="text-gray-800" value="Debut">DEBUT</option>
                </select>
            </div>
            <div class="relative z-0 w-full px-10 mb-5 group">
            <select value="<?= $transactionsData['IdCompte']; ?>" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-400 dark:focus:border-orange-600 focus:outline-none focus:ring-0 focus:border-orange-600 peer" name="IdCompte" id="" >
